...
class Chatroom {
  ...
  removeWhiteListUser (chatroomId, userId) {
    assert(chatroomId && userId, `Paramer 'chatroomId'/'userId' is required`)
    return this.post('user/whitelist/remove', {chatroomId, userId})
  }

  queryWhiteListUser (chatroomId) {
    assert(chatroomId, `Paramer 'chatroomId' is required`)
    return this.post('user/whitelist/query', {chatroomId})
  }

  addWhiteList (objectnames) {
    assert(objectnames, `Paramer 'objectnames' is required`)
    return this.post('whitelist/add', {objectnames})
  }

  removeWhiteList (objectnames) {
    assert(objectnames, `Paramer 'objectnames' is required`)
    return this.post('whitelist/delete', {objectnames})
  }

  queryWhiteList () {
    return this.post('whitelist/query')
  }

  queryUserExist (chatroomId, userId) {
    assert(chatroomId && userId, `Paramer 'chatroomId'/'userId' is required`)
    return this.post('user/exist', {chatroomId, userId})
  }

  queryUsersExist (chatroomId, userId) {
    assert(chatroomId && userId, `Paramer 'chatroomId'/'userId' is required`)
    return this.post('users/exist', {chatroomId, userId})
  }

  addBanUser (userId, minute) {
    assert(userId && minute, `Paramer 'userId'/'minute' is required`)
    return this.post('user/ban/add', {userId, minute})
  }

  removeBanUser (userId) {
    assert(userId, `Paramer 'userId' is required`)
    return this.post('user/ban/remove', {userId})
  }

  queryBanUser () {
    return this.post('user/ban/query')
  }

  addKeepalive (chatroomId) {
    assert(chatroomId, `Paramer 'userId' is required`)
    return this.post('keepalive/add', {chatroomId})
  }

  removeKeepalive (chatroomId) {
    assert(chatroomId, `Paramer 'chatroomId' is required`)
    return this.post('keepalive/remove', {chatroomId})
  }

  queryKeepalive () {
    return this.post('keepalive/query')
  }

...
class Message {
   ...
   * @param opts
   */
  publishTemplate (fromUserId, toUserId, objectName, content, values, pushContent, opts = {}) {
    assert(fromUserId && toUserId && objectName && values && content, new Error('Paramer error'))
    let body = Object.assign({fromUserId, toUserId, objectName, values, content, pushContent}, opts)
  ...
  publishSystemTemplate (fromUserId, toUserId, objectName, values, content, pushContent, opts = {}) {
    assert(fromUserId && toUserId && objectName && content && values && pushContent, new Error('Paramer error'))
    return this.post('system/publish_template', Object.assign({fromUserId, toUserId, objectName, values, content, pushContent}, opts), {contentType: 'application/json'})
  }
